...
public class RMIServiceHost {
    ...
    public static final int RMI_PORT = 420;

    private static PlayerTwoService toggleService;
    private static Registry registry;
    private static PlayerTwoService skeleton;
    ...
    private static void initializeServices() {
        toggleService = new PlayerTwoServiceImpl();
    }

    private static void bindObjects() {
        try {
            registry = LocateRegistry.createRegistry(RMI_PORT);
            skeleton = (PlayerTwoService) UnicastRemoteObject.exportObject(toggleService, RANDOM_PORT_HINT);
            registry.rebind(PlayerTwoService.REMOTE_OBJECT_NAME, skeleton);
            initialized = true;
    ...
    private static void unbindObjects() {
        if (initialized) {
            try {
                registry.unbind(PlayerTwoService.REMOTE_OBJECT_NAME);
